...
Python 3.11。`requirements.txt` のライブラリはバージョンを固定している(各レポートの run_info.json に記録したバージョンと同じ)。更新するときは意図して上げ、レポートとテストを再実行する。
...
python src/live/run_live.py fetch     # Phase 9 実運用用の最新データ → data/live/market/(git 管理外)
python src/live/run_live.py propose   # Phase 9 今日の提案(承認はダッシュボードの ⑧)
python src/live/run_live.py backup    # Phase 9 data/live/ と個人設定のバックアップ(リポジトリの外へ)
python -m pytest                      # テスト
```
...
└─ live.toml                 実運用の既定値(戦略・発注の上限・コスト・税率・経費率)。個人の値は live.local.toml(git 管理外)
src/
├─ data/
...
│  ├─ live_common.py         ⑦〜⑨ の口座の切り替え(デモ / 実運用)
│  └─ tab_*.py               ① Overview 〜 ⑨ Research vs Real の各ページ
└─ live/
   ├─ settings.py            設定(live.toml + live.local.toml)とファイルの場所
   ├─ fx_and_taxes.py        為替・取引コスト・日本の税金(9.1)
   ├─ real_ledger.py         取引台帳から保有・現金・取得原価・損益を再計算(9.2)
   ├─ paper_engine.py        ペーパー(シャドー)トレード(9.3)
   ├─ market.py              実運用用の市場データ(研究データとは別)
   ├─ signal_pipeline.py     データ → シグナル → 目標比率 → 注文 → リスクチェック(9.4)
   ├─ risk_controller.py     フェイルセーフ(9.5)
   ├─ audit.py               監査ログと承認・注文キュー(9.6・9.9)
   ├─ execution_analyzer.py  執行コストとバックテスト・ペーパー・実運用の比較(9.8)
   ├─ attribution.py         損益の要因分解(9.9)
   ├─ bridge.py              一連の流れ(提案・承認後の約定記録・再計算)
   ├─ demo.py                合成データのデモ口座(ダッシュボードとテスト用)
   └─ run_live.py            コマンドライン
tests/                       自動テスト(pytest)
data/
├─ raw/<ticker>.csv          資産ごとの取得データ(研究用、固定)
├─ processed/prices.csv      共通期間の調整後終値(列 = 資産)(研究用、固定)
└─ live/                     実運用の台帳・監査ログ・市場データ(git 管理外。公開リポジトリには入らない)
reports/
...
## Phase 9:実運用への接続(Human-in-the-loop)（完了）

研究結果を実際の少額運用に、安全に・後から検証できる形でつなぐ層。**自動売買は作っていない**(PLAN.md の原則「研究結果から自動的に注文を出すことは当面行わない」)。システムは提案とリスクチェックまでを行い、承認・発注・約定の記録は人が行う。

### 流れ

```
run_live.py fetch      最新の価格と USD/JPY を data/live/market/ に取得(研究データは変えない)
      ↓
提案(propose)          データの確認 → シグナル → 目標比率 → 注文数量(台帳から) → リスクチェック
      ↓                  監査ログに PASS / STOP を記録。PASS ならペーパー口座にも自動で仮想注文
⑧ で承認 / 却下        承認した注文だけが注文キューに入る(STOP と緊急停止中は承認できない)
      ↓
証券会社で自分で発注
      ↓
⑧ で約定を記録         台帳に追加(矛盾する記録は拒否)→ 保有・現金・評価額を再計算
      ↓
⑦ 実資産 / ⑨ 研究と実運用の差
```
...
1. `config/live.local.toml` を作り、自分の値を書く(git 管理外。書いた項目だけが `config/live.toml` を上書きする)。
   ```toml
   [signal]
   strategy = "60/40 annual rebalance"   # backtest.toml の戦略名か "ml"
   [account]
   type = "nisa"                         # "nisa" か "tokutei"
   [limits]
   max_order_jpy = 150000
   ```
2. `python src/live/run_live.py fetch` で最新データを取得する。
3. ダッシュボードの ⑧ で入金を記録し、「今日の提案を作成」(または `run_live.py propose`)。
4. 提案とリスクチェックを確認して承認か却下。承認した注文を証券会社で自分で発注する。
5. 約定したら ⑧ で約定内容(約定日・数量・約定価格・適用レート・手数料)を記録する。配当・手数料・税金も同じ画面で記録する。
6. ⑦ で保有と損益、⑨ でバックテストとの差と損益の要因を確認する。
7. 異常を感じたら ⑧ の「緊急停止」か `run_live.py stop "理由"`。解除は確認のチェックを入れてから。
8. 最後に `python src/live/run_live.py backup`(下の「バックアップ」)。

⑦〜⑨ は「デモ(合成データ)」と「実運用(data/live)」を切り替えられる。デモは研究データで過去1年の運用を再現した合成の口座で、このブラウザのセッションだけの一時フォルダに書き込む(60/40・毎月5万円・整数株・5回に1回は却下、という表示用の設定で、推奨ではない)。

### 実際の資産・口座・取引の情報は公開しない

| 情報 | 置き場所 | git |
|---|---|---|
| 取引履歴・保有・現金・評価額・ペーパー取引・注文キュー・監査ログ・緊急停止 | `data/live/` | 管理外(`.gitignore`) |
| 使う戦略・口座の種類・発注の上限など個人の設定 | `config/live.local.toml` | 管理外 |
| 既定値(上限・コスト・税率・経費率) | `config/live.toml` | 公開(個人の情報は含まない) |

**バックアップ:** `data/live/` と `live.local.toml` は GitHub に無いので、PC を失うと記録も失う。`python src/live/run_live.py backup` で zip を作る(保存先は `live.local.toml` の `[backup] dir`、既定は `~/investment-research-backups`)。PC の外にも残すには、個人の OneDrive など非公開の場所を指定する(学校や会社が管理する場所・共有フォルダは避ける)。保存先がリポジトリの中なら拒否し、書いた zip は読み直して全ファイルの SHA-256 を照合してから残す。古いバックアップは自動では消さない。復元はリポジトリの直下で zip を展開する。

テストとデモは合成データと一時フォルダだけを使い、`data/live/` を読み書きしない。`data/live/` と `live.local.toml` が git の管理外であることもテストで確認している。

### 依頼内容(9.1〜9.10)との対応

| 項目 | 実装 |
|---|---|
| 9.1 為替・コスト・税 | `fx_and_taxes.py`:円 = USD × USD/JPY、仲値 ± スプレッド(既定 25 銭)、手数料、スリッページ、円建てリターンの「価格・為替・交差項」への分解。特定口座(20.315%、同じ年の損益を通算)と NISA(非課税)、配当は米国 10% 源泉のあと特定口座なら 20.315%。経費率は ETF の価格に含まれるので差し引かず、参考として年額を表示 |
| 9.2 台帳 | `real_ledger.py`:DEPOSIT / WITHDRAW / BUY / SELL / DIVIDEND / FEE / TAX。取引履歴だけが正本で、保有数量・現金・取得原価(手数料込みの移動平均)・実現損益・含み損益・評価額の推移は毎回ここから再計算する。売りすぎ・現金のマイナス・二重入力・不正な値を含む追加は丸ごと拒否し、ファイルは変えない |
| 9.3 ペーパー | `paper_engine.py`:PASS の提案はすべて、承認なしでペーパー口座に仮想注文。シグナルの翌営業日の終値(Phase 5 と同じ前提)にスリッページ・為替スプレッド・手数料を加えて約定。実際の入出金をまねるので、「バックテスト → ペーパー」(タイミング・コスト・整数株)と「ペーパー → 実運用」(人の判断・実際の約定)の差を分けて見られる |
| 9.4 パイプライン | `signal_pipeline.py`:Phase 5 の戦略(同じ `decide` のコード)か Phase 7 の ML(今日までに確定したラベルだけで学習し直す)でシグナルを作り、配当込み価格で判断、実際の終値と為替で注文数量を決める。買いのみ(既定)かリバランス、整数株か端数株 |
| 9.5 リスク | `risk_controller.py`:下の表のどれか1つでも該当すれば STOP。理由を `audit_logs/errors.jsonl` に記録 |
| 9.6 承認 | ⑧ Today's Decision:対象銘柄・現在の比率・目標比率・数量・概算金額とリスクチェックを表示し、承認 / 却下。承認した注文だけが注文キューに入る |
| 9.7 実資産 | ⑦ Live Portfolio:評価額・現金・入金累計・損益・含み損益・時間加重リターン、保有明細、評価額の推移、比率、データの日付・承認待ち・未約定・緊急停止・STOP の記録 |
| 9.8 研究と実運用の差 | `execution_analyzer.py` と ⑨:執行コスト = \|約定価格 − バックテストの価格\| × 数量 × 仲値 + 手数料 + 為替コスト。バックテスト・ペーパー・実運用の累積リターン・CAGR・ボラティリティ・最大DD・Sharpe・コスト比率を同じ期間で比較(円建ての時間加重リターン) |
| 9.9 要因分解・監査 | `attribution.py`:損益を「資産のリターン・配分の差・リバランス・為替・配当・手数料と税金・約定」に分解し、合計は毎日の損益と一致する。`audit.py`:提案ごとに decision_id・時刻・data_version・strategy_version・git_commit・シグナル・目標比率・注文・リスクチェック、判断(APPROVED / REJECTED)とメモを追記のみのログに保存 |
| 9.10 テスト | `tests/test_live_bridge.py`(30 件、下記) |

### リスクチェック(1つでも該当すれば STOP)

| 分類 | チェック |
|---|---|
| システム | 緊急停止が設定されている |
| データ | 必要な資産の最新価格が欠損している、0 以下、または `max_data_age_days` 営業日より古い。為替レートがない・0 以下・古い |
| シグナル | シグナルを作れない(履歴不足など) |
| 注文 | 数量が 0 以下、価格のない資産、同じ注文 ID がすでにある(同じ提案内・注文キュー・台帳)、1 日の注文数の上限超え、1 回の金額の上限超え(手数料・スプレッド込み)、保有数を超える売り、現金不足(売りの受取・手数料・スプレッドを含めて計算)、注文後に 1 資産の比率が上限超え |

承認の時点でも、緊急停止と注文の重複をもう一度確認する。承認済みで未約定の注文は約定したものとして次の提案を計算するので、同じ現金で二重に買う提案は出ない。

### 損益の要因分解(`attribution.py`)

毎営業日、前日の保有 q・価格 P(USD)・仲値 X から:

| 要因 | 定義 |
|---|---|
| 資産のリターン | 投資額 × Σ 目標比率 × 価格の騰落率(承認した提案の目標比率で持っていた場合) |
| 配分の差 | 投資額 × Σ(直前の売買後の比率 − 目標比率)× 騰落率。整数株・買いのみ・却下・遅れによるずれ |
| リバランス | 投資額 × Σ(当日の比率 − 直前の売買後の比率)× 騰落率。売買後の値動きによるずれ |
| 為替 | Σ q × P(当日)×(X 当日 − X 前日) |
| 配当 / 手数料・税金 | 台帳の記録そのもの(配当は税引前、源泉税は手数料・税金に入る) |
| 約定 | 約定ごとに ± 数量 ×(当日の終値 × 仲値 − 約定価格 × 適用レート) |

7 つの合計は、毎日の「評価額の変化 − 入出金」と誤差なく一致する(テストで確認)。価格は配当を含まない実際の終値を使うので、配当は配当の行だけに現れる。

### 依頼内容との違い

- **ディレクトリ:** `modules/` ではなく `src/live/`、画面は `src/dashboard/` に追加(Phase 8 と同じ方針)。`data/research/` は作らず、既存の `data/raw/`・`data/processed/` をそのまま研究用(固定)とした。実運用の市場データは `data/live/market/` に別に取得するので、Phase 1〜8 のレポートの再現性は変わらない。
- **証券会社への発注はしない:** API 連携も自動発注もない。承認は注文キューに入れるだけで、発注は人が行う。
- **対象は米国 ETF(USD)と円貨決済:** 投資信託など円建ての資産は未対応。
- **税金は概算:** 外国税額控除、損失の繰越、NISA の枠の管理は含めていない。

### 注意

- **整数株と少額:** VOO は 1 株で約 11.2 万円(2026-09-21 の終値 712.78 ドル × 157.05 円)。整数株しか買えない証券会社で月 1 万円だと、何か月か現金がたまるまで買えない(提案は「注文なし」になる)。`max_order_jpy` も 1 株の金額より大きくしないと、その資産の注文はすべて STOP になる。端数株が使える場合は `fractional_shares = true`。
- **既定の戦略は研究のベンチマーク(Buy & Hold VOO):** どの戦略を使うかは自分で決め、`live.local.toml` に書く。Phase 5〜7 では、どのルール・モデルも Buy & Hold を統計的に上回らなかった。
- **バックテストとの差は、主に現金を持つ期間から生じる:** バックテストは常に全額投資、ペーパーと実運用は整数株で買えない現金と、次の入金までの現金を持つ。ペーパーと実運用の差は、人の判断(却下・遅れ)と実際の約定から生じる。⑨ で確認できる。
- **1 年未満の CAGR は参考程度:** 短い期間の値動きを年率に引き延ばした値。

### テスト(`tests/test_live_bridge.py`、30 件)

- 台帳:手計算との一致(現金・数量・移動平均の取得原価・実現損益・税と手数料)、売りすぎ・現金不足・不正な行・二重入力の拒否、追加の全か無か(失敗したらファイルは1バイトも変わらない)、評価額と時間加重リターン(入金は利益に数えない)、派生ファイルの整合
- 為替・税:スプレッド、コストの見積もり、円建てリターンの分解、NISA と特定口座の配当課税、同じ年の損益通算
- リスク:重複注文、現金を超える買い、1 日の件数・1 回の金額・比率の上限、売りすぎ、数量・資産の不正、古い・欠損・0 以下の価格、為替なし、緊急停止
- 流れ:整数株・買いのみ・リバランスの注文数量、提案 → 承認 → 約定の記録(同じ提案は二重に記録しない、判断は1回だけ、承認前の約定と二重の約定は拒否、承認済み未約定の注文を繰り返さない、監査ログの項目)、STOP と緊急停止中は承認できない、ML は確定したラベルだけで学習
- ペーパー:翌営業日の終値にコストを加えて約定、入金をまねる
- 分析:執行コストの式、比較は共通の期間、要因分解の合計が損益と一致(為替変動・配当・売り・休日の入金・手数料を含む)
- 設定と秘密情報:個人設定の上書き、`data/live/` と `live.local.toml` が git の管理外、バックアップ(中身とチェックサムの照合、リポジトリ内への保存の拒否、壊れたファイルの検出)
- デモと画面:デモで一連の流れ(承認と却下を含む)が矛盾なく動く、⑦〜⑨ がデモでエラーなく表示、緊急停止中は承認ボタンが押せない
